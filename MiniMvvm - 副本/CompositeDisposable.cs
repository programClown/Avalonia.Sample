...
internal sealed class CompositeDisposable : ICollection<IDisposable>, IDisposable
{
    ...
    public CompositeDisposable(params IDisposable[] disposables)
    {
        ...
        // _count can be read by other threads and thus should be properly visible also releases the _disposables
        // contents so it becomes thread-safe
    ...
    private static List<IDisposable?> ToList(IEnumerable<IDisposable> disposables)
    {
        ...
        // do the copy and null-check in one step to avoid a second loop for just checking for null items
    ...
    /// <summary>
    /// Adds a disposable to the <see cref="CompositeDisposable"/> or disposes the disposable if the <see
    /// cref="CompositeDisposable"/> is disposed.
    /// </summary>
    ...
    public void Add(IDisposable item)
    {
        ...
        lock (_gate)
        {
            if (!_disposed)
            {
                ...
                // If read atomically outside the lock, it should be written atomically inside the plain read on _count
                // is fine here because manipulation always happens from inside a lock.
    ...
    public bool Remove(IDisposable item)
    {
        ...
        lock (_gate)
        {
            // this composite was already disposed and if the item was in there it has been already removed/disposed
            if (_disposed)
            {
                return false;
            }

            // List<T> doesn't shrink the size of the underlying array but does collapse the array by copying the tail
            // one position to the left of the removal index. We don't need index-based lookup but only ordering for
            // sequential disposal. So, instead of spending cycles on the Array.Copy imposed by Remove, we use a null
            // sentinel value. We also do manual Swiss cheese detection to shrink the list if there's a lot of holes in it.
        ...
        // if we get here, the item was found and removed from the list just dispose it and report success
    ...
    public void Dispose()
    {
        ...
        lock (_gate)
        {
            if (!_disposed)
            {
                ...
                // nulling out the reference is faster no risk to future Add/Remove because _disposed will be true and
                // thus _disposables won't be touched again.
    ...
    /// <paramref name="arrayIndex"/> is less than zero. -or - <paramref name="arrayIndex"/> is larger than or equal to
    /// the array length.
    /// </exception>
    public void CopyTo(IDisposable[] array, int arrayIndex)
    {
        ...
        lock (_gate)
        {
            ...
            {
                // there is not enough space beyond arrayIndex to accommodate all _count disposables in this composite
                throw new ArgumentOutOfRangeException(nameof(arrayIndex));
            }
    ...
    public IEnumerator<IDisposable> GetEnumerator()
    {
        lock (_gate)
        {
            ...
            // the copy is unavoidable but the creation of an outer IEnumerable is avoidable
    ...
    /// <summary>
    /// An empty enumerator for the <see cref="GetEnumerator"/> method to avoid allocation on disposed or empty composites.
    /// </summary>
    ...
    private sealed class CompositeEnumerator : IEnumerator<IDisposable>
    {
        ...
        public void Dispose()
        {
            // Avoid retention of the referenced disposables beyond the lifecycle of the enumerator. Not sure if this
            // happens by default to generic array enumerators though.
